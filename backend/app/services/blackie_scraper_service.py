"""Servei de volcat d'autors de Blackie Books (agressiu, multi-worker).

- Recull tots els slugs del índex `/autores/` (una sola pàgina).
...
from app.clients.blackie_client import BlackieClient, RateLimitedError
from app.crud.blackie_repository import BlackieRepository
...
class BlackieScraperService:
    """Orquestador del volcat: índex → perfils → BD.
    ...
    # ------------------------------------------------------------------ #
    async def collect_slugs(self) -> list[str]:
        try:
            return await self.client.get_authors_index()
        except RateLimitedError as exc:
            self._notify_rate_limit("/autores/", str(exc))
            return await self.client.get_authors_index()
    ...
    async def scrape(
        ...
        slugs = await self.collect_slugs()
        if not refresh:
            async with self.session_factory() as db:
                fetched = await BlackieRepository(db).fetched_slugs()
        ...
        total = len(pending)
        stats = ScrapeStats()
        print(f"[blackie] {len(slugs)} autores en índex | a processar: {total}", flush=True)

        semaphore = asyncio.Semaphore(self.workers)

        async def process(slug: str, repo: BlackieRepository) -> None:
                    ...
                    try:
                        ...
                        async with done_lock:
                            ...
                            if done % progress_every == 0 or done == total:
                                print(
                                    f"[blackie] {done}/{total} | ok={stats.ok} "
